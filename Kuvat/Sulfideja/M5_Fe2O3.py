...
I0_ini = ([224],[232,233])
I_m5_ini = (range(220,224),[218,219])

I0_ind = ([201,218],[117,118])
m1_ind = ([189,192,195,198],[187,188])
metal1_ind = ([206,209,212,215],[204,205])
I0s_ind = ([309,326],[225,226])
m1s_ind = ([297,300,303,306],[295,296])
metal1s_ind = ([314,317,320,323],[312,313])

I0_ind2 = ([202,219],[117,118])
m1_ind2 = ([190,193,196,199],[187,188])
metal2_ind = ([207,210,213,216],[204,205])
I0s_ind2 = ([293,327],[225,226])
m1s_ind2 = ([298,301,304,307],[295,296])
metal2s_ind = ([315,318,321,324],[312,313])

I0_ind3 = ([203,220],[117,118])
m1_ind3 = ([191,194,197,200],[187,188])
metal3_ind = ([208,211,214,217],[204,205])
I0s_ind3 = ([294,328],[225,226])
m1s_ind3 = ([299,302,305,308],[295,296])
...
helxas_ref.read_I('M5_dry',I_m5_ini[0],I_m5_ini[1],40)

helxas_ref.set_background_fit_order(3,False)
e_dry, mux_dry, mux_err_dry = helxas_ref.get_spectrum('M5_dry')
...
mux_s2s = mux_s2s - np.polyval(n_s2s,fit_e)-0.015
...
#mux_s2s = mux_s2s*(1+0.0021*(e_s2s-7175))
#mux_FeS = mux_FeS*(1-0.0003*(e_s2s-7200))
...
#fit = outlier_fit(model, [0.2,0.8], e_s2s,mux_s2s, mux_s2s_err, method='cauchy')
...
plt.plot(e_s2,mux_s2, label='M5 C0 S1 a') 
plt.plot(fit_e,mux_dry, label='M5 dry')
#plt.plot(e_s2s,mux_s2s, label='M5 C1 S1 b')
...
#plt.plot(fit_e, model(fit_e,fit.p[0],fit.p[1]), label='fit FeS = %.4f $\pm$ %.4f, M5 = %.4f $\pm$ %.4f' %(res_p[0], res_cov[0], res_p[1], res_cov[1]))
