...
I0_ini = ([182],[160,161])
I_m4_ini = (range(178,182),[176,177])


I0_ind = ([184,218],[117,118])
m1_ind = ([172,175,178,181],[170,171])
metal1_ind = ([206,209,212,215],[204,205])
I0s_ind = ([292,326],[225,226])
m1s_ind = ([280,283,286,289],[278,279])
metal1s_ind = ([314,317,320,323],[312,313])

I0_ind2 = ([185,219],[117,118])
m1_ind2 = ([173,176,179,182],[170,171])
metal2_ind = ([207,210,213,216],[204,205])
I0s_ind2 = ([293,327],[225,226])
m1s_ind2 = ([281,284,287,290],[278,279])
metal2s_ind = ([315,318,321,324],[312,313])

I0_ind3 = ([186,220],[117,118])
m1_ind3 = ([174,177,180,183],[170,171])
metal3_ind = ([208,211,214,217],[204,205])
I0s_ind3 = ([294,328],[225,226])
m1s_ind3 = ([282,285,288,291],[278,279])
...
helxas_ref.read_I('M4_dry',I_m4_ini[0],I_m4_ini[1],40)

helxas_ref.set_background_fit_order(3,False)
e_dry, mux_dry, mux_err_dry = helxas_ref.get_spectrum('M4_dry')
...
mux_s2 = mux_s2 - np.polyval(n_s2,fit_e)-0.01
...
#Normalisointi
mux_s2s = mux_s2s*(1+0.0006*(e_s2s-7220))
mux_s2s_err = mux_s2s_err/np.mean(mux_s2s[np.logical_and(e_s2s<7230,e_s2s>7220)])


mux_s2 = mux_s2/np.mean(mux_s2[np.logical_and(e_s2<7230,e_s2>7180)])
mux_s2s = mux_s2s/np.mean(mux_s2s[np.logical_and(e_s2s<7230,e_s2s>7180)])
mux_FeS = mux_FeS/np.mean(mux_FeS[np.logical_and(fit_e<7230,fit_e>7180)])
mux_dry = mux_dry/np.mean(mux_dry[np.logical_and(fit_e<7230,fit_e>7180)])
mux_Fe2O3 = mux_Fe2O3/np.mean(mux_Fe2O3[np.logical_and(fit_e<7230,fit_e>7180)])
...
plt.plot(e_s2,mux_s2, label='M4 C0 S1 a') 
plt.plot(fit_e,mux_dry, label='M4 dry')
#plt.plot(e_s2s,mux_s2s, label='M4 C1 S1 a')
...
#plt.plot(fit_e, model(fit_e,fit.p[0],fit.p[1]), label='fit Fe2O3 = %.4f $\pm$ %.4f, M4 = %.4f $\pm$ %.4f' %(res_p[0], res_cov[0], res_p[1], res_cov[1]))
